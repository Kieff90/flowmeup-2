...
export function VideoLoop({ lang = 'it' }: VideoLoopProps) {
  ...
  return (
    <section aria-label={label} className="bg-ink-950">
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: '16 / 9' }}>
        {/* Placeholder — visible when video has no src */}
        <div
          aria-hidden="true"
          className="absolute inset-0 flex flex-col items-center justify-center bg-ink-950"
          style={{ border: '1px solid rgba(255,255,255,0.06)' }}
        >
          <span className="font-heading text-[clamp(32px,6vw,72px)] font-black tracking-[0.18em] text-ink-800">
            VIDEO
          </span>
          <span className="mt-3 font-heading text-xs font-bold uppercase tracking-[0.2em] text-ink-900">
